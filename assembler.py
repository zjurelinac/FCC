...
def define_data( cmd, args, n ):
    ...
    while args:
        ...
        for i in range( 0, size ):
            ...
            memory[ n ] = y
            n += 1
    group_size = data['consts']['words_per_line'] // data['consts']['define_data'][cmd]
    np = [concat(p[i:i+group_size]) for i in range(0, len(p), group_size)]
    np[-1] += '0' * (32 - len(np[-1]))
    return np
...
# Function
def parse_lines( ls, start_num = 0 ):
    lines = []
    num = start_num
    for l in ls:
        res = { 'original' : l }
...
# Main function
def assemble(f, start_num=0, generate_symbol_table=False):
    ...
    generate_symbol_table = bool(generate_symbol_table)
    ...
    try:
        pls = parse_lines( open( f ).read().splitlines(), start_num )
    ...
    if generate_symbol_table:
        with open('resources/sym_table.a', 'w') as f:
